...
def Train(model, train_loader, valid_loader, criterion, optimizer, DEVICE, 
    ...
    for epoch in range(max_epochs):
        ...
        valid_loss_fi = valid_loss / len(valid_loader.dataset)
        valid_accuracy = 100. * correct / total
        valid_losses.append(valid_loss_fi)
        valid_accs.append(valid_accuracy)
        
        print(f"Epoch [{epoch+1}/{max_epochs}] - "
              f"Train Loss: {train_loss:.4f}, Valid Loss: {valid_loss_fi:.4f},Valid Acc: {valid_accuracy:.2f}%")
        wandb.log({"Train Loss": train_loss, "Valid Loss": valid_loss_fi, "Valid Acc": valid_accuracy}, step=epoch+1)
